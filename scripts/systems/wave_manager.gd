...
func _spawn_one() -> void:
	...
	if wave >= 4:
		lasers = min(3, wave - 3)
	var scene
	# _to_spawn é decrescente: as últimas vagas da wave viram laser,
	# depois dasher, depois shooter, o resto swarmer.
	if _to_spawn <= lasers:
		scene = LaserShooterScene
	elif _to_spawn <= lasers + dashers:
		scene = DasherScene
	elif _to_spawn <= lasers + dashers + shooters:
		scene = ShooterScene
	else:
		scene = SwarmerScene
