extends CanvasLayer
const MAX_SPLATS := 560
const WIPE_PASSES := 3
const MAX_DIRT := 1.0
const MIN_SPLAT_DISTANCE := 38.0  ## distancia minima entre centros de manchas
...
func _stamp(direction: Vector2, weapon: WeaponData) -> void:
	if _dirt_total >= MAX_DIRT:
		return
	var screen := get_viewport().get_visible_rect().size
	var center := screen * 0.5
	var pos := _find_placement(screen, center, direction)
	if pos == Vector2.INF:
		return
	...
	_dirt_total = minf(MAX_DIRT, _dirt_total + weapon.splat_dirtiness)
	if _splats.size() > MAX_SPLATS:
		_remove_oldest(1)
	else:
		_emit_dirtiness()
## Tenta achar uma posicao que nao fique muito perto de nenhuma mancha
## existente. Se todas as tentativas falharem, usa a menos ruim (maior
## distancia da mancha mais proxima) em vez de sobrepor totalmente.
func _find_placement(screen: Vector2, center: Vector2, direction: Vector2) -> Vector2:
	...
	for attempt in PLACEMENT_ATTEMPTS:
		...
		var closest := _closest_splat_distance(pos)
		if closest >= MIN_SPLAT_DISTANCE:
			return pos
		if closest > best_min_dist:
			best_min_dist = closest
			best_pos = pos
	return best_pos
func _closest_splat_distance(pos: Vector2) -> float:
	var closest := INF
	for entry in _splats:
		var node: Node2D = entry.node
		if not is_instance_valid(node):
			continue
		var d := pos.distance_to(node.position)
		if d < closest:
			closest = d
	return closest
