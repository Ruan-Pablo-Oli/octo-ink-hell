...
const ShieldUpgradeData := preload("res://resources/upgrades/rotating_shield.tres")
...
func _setup_sprite() -> void:
	...
	# Make sure the shader starts disabled.
...
func _recompute() -> void:
	...
	_dash_invincible = upgrades.has_flag(S.DASH_INVINCIBLE)

	# If the upgrade gets removed/disabled while not dashing,
	# make sure the shader is turned off.
	...
	if _shield:
		_shield.set_stack_count(
			upgrades.stacks_of(ShieldUpgradeData)
		)
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("toggle_tool"):
		_set_tool(
			Tool.ATTACK if _tool == Tool.CLEAN else Tool.CLEAN
		)
	...
	if _dash_timer > 0.0:
		...
		# Keep the effect active while invincible dash is running.
		if _dash_invincible:
	...
	# =========================================================
		...
		# Activate shader only if Invincible Dash is active.
		if _dash_invincible:
			_set_dash_shader(true)
	...
	# =========================================================
		...
		# Safety: shader must not remain active outside the dash.
		_set_dash_shader(false)
...
func take_damage(amount: float) -> void:
	...
	# Invincible Dash
	if _dash_timer > 0.0 and _dash_invincible:
		return
...
func _die() -> void:
	...
	# Make sure the effect is disabled when the player dies.
	_set_dash_shader(false)
